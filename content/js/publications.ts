 ...
 */
const updateURL = (history: History, intialized: boolean = true) => {
...
const Publications = () => {
    ...
    const updateList = () => {
        ...
        forEach(['author', 'subject', 'publication-type'], (element: string) => {
            let current: string = ''
            $(`.tag-list__tag--${element}.tag-list__tag--active[data-tag]`).each((index, item) => {
        ...
        const authors = foundTags['author'].length > 0 ? foundTags['author'].split(' ') : []
        const subjects = foundTags['subject'].length > 0 ? foundTags['subject'].split(' ') : []
        const publications = foundTags['publication-type'].length > 0 ? foundTags['publication-type'].split(' ') : []

        publicationList.filter((item) => {
            const values: PublicationTile = item.values() as PublicationTile
            const itemtags = filter(values['tagslugs'].trim().split(' '), (t) => (!(t === '')))

            const a = authors.length > 0
                      ? intersection(authors, itemtags).length === authors.length
                      : true
            const b = subjects.length > 0
                      ? intersection(subjects, itemtags).length === subjects.length
                      : true
            const c = publications.length > 0
    ...
    publicationList.on('updated', () => {
        ...
        forEach(publicationList.visibleItems, (el, index) => {
        ...
        $('.tag-list__tag').each((index: number, item: HTMLElement) => {
            ...
            if (typeof tag !== 'undefined') {
                ...
                if (tag !== '' && indexOf(visibleTags, tag) === -1) {
                    $(item).addClass('tag-list__tag--disabled').removeClass('tag-list__tag--active')
                } else {
    ...
    // Tag buttons
    $('.tag-list__tag').click(function(this: HTMLElement, e) {
