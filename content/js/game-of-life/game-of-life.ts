...
const gameOfLife = () => {
    ...
    $('.animation-controls__header button').on('click', (e) => {
        $('.animation-controls').toggleClass('open')
    })

    $('.animation-controls__input').on('change keyup', function(this: HTMLElement, e) {
    ...
    const changeValue = (el: HTMLElement) => {
        let val: number = parseInt($('.animation-controls__input').val(), 10) + ($(el).is('.up') ? 1 : -1)
    ...
    $('.animation-controls__control button').on('mousedown', function(this: HTMLElement, e) {
